...
const INSTRUCTION_LINES = [
  ...
  'The wall: when the app says the wall of tiles is showing, the learner can open a lesson or one of its chapters (dashboard_open_tile), sort the wall (dashboard_filter), open settings (open_settings) or replay the rundown (replay_rundown). The six coming-soon worlds cannot be opened and have no date: if asked, say so in one plain sentence and never promise when.',
  'The rundown: while the app runs the rundown it gives you each stop as exact words. Say them word for word and stop. Do not add steps, hints or praise. If the learner asks a question during it, answer in one sentence and stop.',
...
export const TOOLS = [
  ...
  { type: 'function', name: 'open_lesson', description: 'Call when the lesson cards are showing and the learner asks to open or start a lesson. The app opens it exactly as if the learner pointed at the card and returns the first step.',
    parameters: { type: 'object', properties: { cardId: { type: 'string', enum: CARD_IDS } }, required: ['cardId'], additionalProperties: false } },
  { type: 'function', name: 'dashboard_open_tile', description: 'Call when the wall of tiles is showing and the learner asks to open a lesson or a chapter: "open Cargo Crew", "chapter 3 of the café", "continue". chapter 0 = the lesson from its briefing, 1 to 5 = that chapter directly. Coming-soon worlds cannot be opened; the app returns ok false and the reason to say.',
    parameters: { type: 'object', properties: { lesson_id: { type: 'string', enum: [...CARD_IDS, 'continue'], description: '"continue" = where the learner left off; chapter is then ignored.' }, chapter: { type: 'integer', minimum: 0, maximum: 5 } }, required: ['lesson_id', 'chapter'], additionalProperties: false } },
  { type: 'function', name: 'dashboard_filter', description: 'Call when the wall is showing and the learner asks to sort it: "show me the newest", "what do I open most", "back to featured". The app switches the wall to that filter and returns ok.',
    parameters: { type: 'object', properties: { filter: { type: 'string', enum: ['featured', 'newest', 'most_viewed'] } }, required: ['filter'], additionalProperties: false } },
  { type: 'function', name: 'open_settings', description: 'Call when the learner asks to open settings, change the room, the language, captions or volume. The app opens the settings card, the same as pressing the gear, and returns ok.',
    parameters: NO_PARAMS },
  { type: 'function', name: 'replay_rundown', description: 'Call when the learner asks how this works, to see the tutorial again or to replay the rundown. Works from the wall only; the app runs the rundown again and returns ok and a reason.',
    parameters: NO_PARAMS },
