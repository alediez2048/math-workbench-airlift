...
export const INSTRUCTIONS = [
  ...
  'Cargo Crew story: the learner is the load planner at Dock 7, a busy harbor. Ships bring full crates; trucks, pickups and vans each take an equal share of one container. Use this story language: crates, one container, trucks, pickups, vans, loading. Never give real loading or safety advice.',
...
export const TOOLS = [
  ...
  { type: 'function', name: 'split_cargo', description: 'Call when the learner says "split it", "cut it in half", "split the crates" or "make smaller chunks". The app splits the loose crates into equal chunks if this chapter allows it and nothing is held, and returns ok, reason and what is on the table.',
    parameters: NO_PARAMS },
  { type: 'function', name: 'check_load', description: 'Call when the learner says "load it", "check my load", "is this right", "send it" or "done". The app checks the crates on the container floor and returns ok, reason, feedback and, when accepted, the expression. This is the only source of whether a load is correct.',
    parameters: NO_PARAMS },
  { type: 'function', name: 'reset_cargo', description: 'Call when the learner says "put everything back", "reset" or "take the crates off". The app moves the loose crates off the container floor, keeping their current size, and returns ok and a reason.',
    parameters: NO_PARAMS },
